...
<script type="text/javascript">
	...
	var page = 1;
		...
		function load(search, page){
			...
			$.ajax({
				type : "GET",
				url : "http://localhost:8080/quanlysinhvien_jax-rs/webapi/sinhvien/showbypage?search="+search+"&page="+page,
				success : function(result) {
					if (result != null) {
						...
						$.each(result, function(key, value) {
							...
							$("#dataTable").append('<tbody><tr ><td>'+value.masinhvien+'</td><td>'+value.hoten+'</td><td>'+(value.gioitinh == 1 ? "Nam" : "Nữ")+'</td><td>'+value.ngaysinh+'</td><td>'+value.diachi+'</td><td>'+value.email+'</td><td>'+value.sodienthoai+'</td><td><button data-id="'+value.masinhvien+'" class="sua btn btn-primary">Sửa</button><button data-id="'+value.masinhvien+'" class="xoa btn btn-success">Xóa</button></td></tr></tbody>');
							$(".sua").off('click').on(
									'click',
									function(e) {
										var x = $(this).data('id');
										sessionStorage.setItem("masinhvien", x);
										window.location.replace("http://localhost:8080/quanlysinhvien_jax-rs/quan-ly-sinh-vien?action=suasinhvien");
										});
							$(".xoa").off('click').on(
									'click',
									function(e) {
										$('#xoaModal').modal('show');
										var y = $(this).data('id');
										$("#srcXoa").val("http://localhost:8080/quanlysinhvien_jax-rs/webapi/sinhvien/delete/"+y);
			...
			$.ajax({
				type : "GET",
				url : "http://localhost:8080/quanlysinhvien_jax-rs/webapi/sinhvien/phantrang?search="+search,
				success : function(result) {
					if (result != null) {
					...
					<div id="dataTable_wrapper"
						...
						<div class="row">
							...
							<button class="btn btn-primary"id="submittk">
								...
								<a
									href="/quanlysinhvien_jax-rs/quan-ly-sinh-vien?action=themmoisinhvien"
